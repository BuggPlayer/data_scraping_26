...
progress = {
    ...
    "percent": 0,
    "file_ready": False,
    "output_file": "amc_leads.xlsx"
}

# -----------------------
# LEAD SCORING ENGINE (AMC‑focused)
# -----------------------
def calculate_lead_score(result: dict, biz_name: str, biz_types: list) -> int:
    score = 0
    ...
    if "amc" in name_lower or "annual maintenance" in name_lower:
        score += 15
    # Common Google place types for repair / service businesses
...
def scrape_leads(api_key: str, queries: List[str], location: str):
    ...
    try:
        ...
        for i, biz in enumerate(unique):
            ...
            # Use the raw name and types for scoring
            biz_name = result.get("name", biz["name"])
            biz_types = result.get("types", biz.get("types", []))
            score = calculate_lead_score(result, biz_name, biz_types)
...
def start_scrape():
    ...
    # Default AMC‑focused categories
    if not categories:
        categories = [
            "AC repair AMC",
            "Refrigerator repair",
...
<head>
  ...
  <title>AMC Lead Generator</title>
  <style>
...
<div class="container">
  <div class="card">
    <h1>AMC <span>Lead Generator</span></h1>
    <p class="sub">Find businesses that sell Annual Maintenance Contracts.</p>
    ...
    <textarea id="categories" rows="8">AC repair AMC
Refrigerator repair
Washing machine repair
TV repair
Water purifier service
...
RO service
Chimney repair
Electrical appliance repair</textarea>
